...
class _PreferencesPageState extends State<PreferencesPage> {
  ...
  void fetchRecommendedCourses() async {
    ...
    final response = await http.post(
      ...
      body: json.encode({
        'customSearch': customFieldController.text.toLowerCase(),
    ...
    if (response.statusCode == 200) {
      ...
      if (courses.isEmpty) {
        ...
        setState(() {
          recommendedCourseTitles = courses.map((course) => course['Title'].toString()).toList();
          recommendedCourseSummary = courses.map((course) => course['Summary'].toString()).toList();
          recommendedCourseURL = courses.map((course) => course['course_url'].toString()).toList();
        });
      }
    } else {
